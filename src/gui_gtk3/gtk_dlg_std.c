...
#endif
/*!
\file ../gui_gtk3/gtk_dlg_std.c
\brief standard-dialogs, msgbox, slider, color-selection
\code
...
#include "../gui_gtk3/gtk_dlg_std.h"
...
// Gtk2: no callback of "release Mousbutton"; gcad: use UI_GR_ButtonM1Release
...
static MemObj thisWin;
  ...
  thisWin = GUI_Win__ ("", GUI_DialogEntryCB, "");
  // box0 = GUI_box__ (&UI_DialogEntryWin, 1, 0, 0);
  box0 = GUI_box_v (&thisWin, "");
  GUI_spc__ (&box0, 1, 4);
  ...
  GUI_Win_go (&thisWin);
  GUI_Win_up (NULL, &thisWin, 1);
  // GUI_update__ ();


  // wait ..
  OS_file_sig_wait (2, (void*)&irc);
    // printf(" _DialogEntry irc=%d\n",irc);


  // get entry back
  // if(GTK_IS_WINDOW(thisWin)) {        // delete destroys win0 
  if(irc >= 0) {
    if(entry) {
      ...
      entry[eSiz] = '\0';
        // printf(" _DialogEntry |%s| %d\n",entry,eSiz);
    }

    // GUI_set_show (&thisWin, 0);  // hide  2012-09-02
    GUI_Win_kill (&thisWin);
    // GUI_update__ ();
  }     
...
/// change slider-value
  ...
  // ((GtkAdjustment*)wo)->value = newVal;
  // update Slider
  // gtk_adjustment_value_changed ((GtkAdjustment*)wo);
